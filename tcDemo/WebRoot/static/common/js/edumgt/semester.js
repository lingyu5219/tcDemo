$(function () {
	var tableOption = {
		//ajax查询数据的http请求url
		queryUrl : basePath + "/semester/querySemesterList",
		//ajax删除数据的http请求url
		delUrl : basePath + "/semester/deleteSemester",
		//ajax增加数据的http请求url
		addUrl : basePath + "/semester/addSemester",
		//Table对象
		table : $('#dataListTable'),
		//查询表单Form对象
		form : $("#queryForm"),
  ...
  //增加
    $("#btn-add").on("click", function () {
    	var jcAdd = $.dialog({
    		title: '增加学期',
    	    content: 'url:' + basePath + '/forward?page=edumgt/semesterAdd',
    ...
    tableOption.table.on("click",".delRow",function() {
        ...
        var params = {semesterId:item.semesterId};
        //调用删除方法，向服务器发送请求
