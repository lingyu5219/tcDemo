...
//										}
	...
	//调整人员显示区域高度
	var memAreaHeight = $(".container").height();
	var treeMenuHeight = $(".nav").height();
	//alert(memAreaHeight + "," + treeMenuHeight);
	if(memAreaHeight < treeMenuHeight){
		$(".container > .content .leo-content").css("min-height",treeMenuHeight-248);
		$(".container > .content").css("border-bottom-left-radius","0px");
	}
});
			...
			if(result.result == "1"){
				...
				$("#dept-tree").leoTreeMenu({
					...
					nodeClick:function(node){
						...
						window.location.href = ctx + "/enterprise/organization_management.html?param=" + node.department_id;
					}
				});
			} else {
				leoAlert(result.message);
...
//调用接口：/api/enterprise/departmentmember!getMemberListByDepartmentID.do?member_list_type=faceforassign

//调用接口 获取页面负责人、助理、成员 ：/api/enterprise/department!getDepartmentWithMemberListIncludePageInfo.do
function getMemberByDeptIdWithPage(deptId,page){
	curDeptIdSelected = deptId;
	//alert("当前所选部门ID:" + curDeptIdSelected);
	$.ajax({
		url:ctx + "/api/enterprise/departmentmember!getMemberListByDepartmentID.do?member_list_type=facefordepartment",
		data:{
		...
		success:function(result){
			...
			if(result.result == 1){
				...
				for(var i=0; i < members.length; i++){
					...
					if(br == 0){
						if(lineWrap != null){
							lineWrap.append($("<div class=\"clear\">"));
						}
						lineWrap = $("<div class=\"leo-sticker-wrap-withborder\">");
					}
					...
					var sticker = new LeoSticker({
						...
						noHover:true,
						splitTop: false,
						splitRight: i % 5 != 4 && i != members.length - 1 ? true : false, 
						delBtnClick: function(e){
							delEmployee(memberItem);
						}
					});
					lineWrap.append(sticker);
					...
					if(br == 0){
						$("#unallocated-member").append(lineWrap);
					}
				}

				$("#unallocated-member-pagination").leoPagination({
					totalPage:totalPage,
					curPage:curPage,
					goPage:function(num){
						getMemberUnallocated(num);
					}
				});
				//调整人员显示区域高度
				var memAreaHeight = $(".container").height();
